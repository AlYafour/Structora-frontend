...
/* ────────────────────────────────────────────────────────
 * Small helper: info card used in the summary grid
 * ──────────────────────────────────────────────────────── */
function InfoCard({ label, value }) {
  if (!value) return null;
  return (
    <Card className="ds-info-card">
      <div className="prj-info-label">{label}</div>
      <div className="prj-info-value">{value}</div>
    </Card>
  );
}

const SitePlanTab = memo(function SitePlanTab({ projectId, siteplan }) {
  ...
  // Build Google Maps link from siteplan data
  const mapsData = useMemo(() => {
    ...
    // Need either coordinates or zone/sector data
    if (!hasCoords && !zone && !sector && !landNo) return null;

    // Display label for the share text
    const label = [zone, sector, landNo ? `Plot ${landNo}` : "", plotAddress]
      .filter(Boolean)
      .join(" - ");

    if (hasCoords) {
      // Exact coordinates — pin on the plot
    ...
    // Fallback: text search (neighborhood level)
  ...
  /* ── Action buttons (shared by empty & populated states) ── */
  const actions = (
  ...
  /* ── Owners with id_attachment ── */
  const ownersWithId =
    siteplan?.owners?.filter((o) => o.id_attachment) || [];

  return (
    <div className="prj-tab-panel">
      {actions}

      {/* ─── Plot Image + Map (hero section) ─── */}
                ...
                {shareMenuOpen && (
                  <div className="siteplan-share-menu">
                    <button
                      ...
                      onClick={() => {
                        window.open(
                          `https://wa.me/?text=${encodeURIComponent(t("plot_location_share_text") + "\n" + mapsData.label + "\n" + mapsData.url)}`,
                          "_blank"
                        );
      ...
      {/* ─── Summary info grid ─── */}
      <div className="ds-mt-6 ds-grid-auto-240">
        <InfoCard label={t("municipality")} value={siteplan?.municipality} />
        <InfoCard label={t("zone")} value={siteplan?.zone} />
        <InfoCard label={t("sector")} value={siteplan?.sector} />
        <InfoCard label={t("plot_area_sqm")} value={siteplan?.plot_area_sqm} />
        <InfoCard label={t("plot_area_sqft")} value={siteplan?.plot_area_sqft} />
        <InfoCard label={t("land_no")} value={siteplan?.land_no} />
        <InfoCard
          label={t("allocation_type")}
          value={siteplan?.allocation_type && toLocalizedUse(siteplan.allocation_type, i18n.language)}
        />
        <InfoCard
          label={t("land_use")}
          value={siteplan?.land_use && (LAND_USE_LABEL_KEYS[siteplan.land_use] ? t(LAND_USE_LABEL_KEYS[siteplan.land_use]) : toLocalizedUse(siteplan.land_use, i18n.language))}
        />
        <InfoCard
          label={t("allocation_date")}
          value={siteplan?.allocation_date && formatDate(siteplan.allocation_date, i18n.language)}
        />
        <InfoCard
          label={t("application_date")}
          value={siteplan?.application_date && formatDate(siteplan.application_date, i18n.language)}
        />
        <InfoCard label={t("application_number")} value={siteplan?.application_number} />
      </div>
            ...
            {ownersWithId.map((owner, index) => {
              ...
              return (
                <div key={owner.id || owner.id_number || index}>
                  ...
                  <FileAttachmentView
                    fileUrl={typeof idAttachment === "string" && idAttachment.trim() ? idAttachment : null}
                    fileName={typeof idAttachment === "string" ? extractFileNameFromUrl(idAttachment) : ""}
                    projectId={projectId}
                    endpoint={`projects/${projectId}/siteplan/`}
                  />
                </div>
