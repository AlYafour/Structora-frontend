...
const VariationItemsTable = memo(({
  ...
  const totalOmitted = omittedItems.reduce(
    (sum, item) => sum + (parseFloat(item.amount) || 0),
    0
  );

  const totalAdded = addedItems.reduce(
    (sum, item) => sum + (parseFloat(item.amount) || 0),
    0
  );
    ...
    <div className="nvt-wrapper nvc-section">
      <div className="nvt-header">
        <div className="nvt-toggle">
          <button
            type="button"
            className={`nvt-toggle__btn nvt-toggle__btn--omit ${
              isOmitted ? 'nvt-toggle__btn--active' : ''
            }`}
          ...
          <button
            type="button"
            className={`nvt-toggle__btn nvt-toggle__btn--add ${
              !isOmitted ? 'nvt-toggle__btn--active' : ''
            }`}
        ...
        <div className="nvt-header__right">
          <div className="nvt-header__summary">
            ...
            <span
              className={`nvt-header__summary-value ${
                isOmitted ? 'nvt-val--neg' : 'nvt-val--pos'
              }`}
            >
          ...
          {isEditMode && (
            <button
              type="button"
              className={`nvt-add-btn ${
                isOmitted ? 'nvt-add-btn--omit' : 'nvt-add-btn--add'
              }`}
              onClick={(e) => {
                e.preventDefault();
                onAdd();
      ...
      <div className="nvt-table-wrap">
        <table className="nvt-table">
          <colgroup>
            <col className="nvt-col-desc" />
            <col className="nvt-col-qty" />
            <col className="nvt-col-unit" />
          ...
          <thead>
            <tr
              className={`nvt-thead ${
                isOmitted ? 'nvt-thead--omit' : 'nvt-thead--add'
              }`}
            >
              <th className="nvt-th nvt-th--desc">
                {t('item_description')}
              </th>

              <th className="nvt-th nvt-th--num">
                {t('quantity')}
              </th>

              <th className="nvt-th nvt-th--unit">
                {t('unit')}
              </th>

              <th className="nvt-th nvt-th--rate">
                {t('rate')}
              </th>

              {isOmitted && (
                <th className="nvt-th nvt-th--ohp">
                  {t('omitted_includes_overhead_profit')}
                </th>
              )}
              ...
              {isEditMode && (
                <th className="nvt-th nvt-th--action no-print" />
              )}
            </tr>
          </thead>
            ...
            {items.length === 0 ? (
              <tr>
                <td colSpan={colSpan} className="nvt-td nvt-empty">
                  <span className="nvt-empty__icon">
                    {isOmitted ? '⊖' : '⊕'}
                  </span>

                  <span className="nvt-empty__text">
                    {t('no_items')}
                  </span>
      ...
      <div
        className={`nvt-footer ${
          isOmitted ? 'nvt-footer--omit' : 'nvt-footer--add'
        }`}
      >
        ...
        <div className="nvt-footer__active">
          ...
          <span
            className={`nvt-footer__active-val ${
              isOmitted ? 'nvt-val--neg' : 'nvt-val--pos'
            }`}
          >
            {formatCurrency(activeTotal)}
