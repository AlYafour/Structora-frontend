...
export default function LicenseStep({ projectId, onPrev, onNext, isView: isViewProp, isNewProject = false, onLicenseReady, isActive = true, onDocFilesChange, onFormSectionChange, hasBlockingErrors = false, prefillData = null }) {
  ...
  const [activeStampConsultantId, setActiveStampConsultantId] = useState(null);
    ...
    (async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          const stampConsultantId = licenseData.supervision_consultant?.id || licenseData.design_consultant?.id || null;
          if (stampConsultantId) setActiveStampConsultantId(stampConsultantId);
    ...
    await runSave(async () => {
      ...
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          // Resolve which consultant owns the stamp (supervision first, fallback to design)
          const stampConsultantId = licenseData.supervision_consultant?.id || licenseData.design_consultant?.id || activeStampConsultantId;
          if (stampConsultantId) setActiveStampConsultantId(stampConsultantId);

          // If user uploaded a new stamp, PATCH the consultant profile
          if (consultantStamp instanceof File && stampConsultantId) {
            try {
              const stampFd = new FormData();
              stampFd.append("stamp", consultantStamp);
              const { data: updatedConsultant } = await api.patch(`consultants/${stampConsultantId}/`, stampFd);
              if (updatedConsultant?.stamp_url) {
                setExistingConsultantStampUrl(updatedConsultant.stamp_url);
              }
            } catch (stampErr) {
              logger.error("Error updating consultant stamp", stampErr);
            }
          }
          ...
          <div className="wizard-section-card wizard-mt-4">
            ...
            <PersonField
              ...
              onSelect={(c) => {
                ...
                if (c.id) setActiveStampConsultantId(c.id);
                if (c.stamp_url) setExistingConsultantStampUrl(c.stamp_url);
              }}
